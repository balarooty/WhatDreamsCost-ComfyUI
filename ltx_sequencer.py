...
class LTXSequencer(LTXVAddGuide):
    ...
    def define_schema(cls):
        inputs = [
            ...
            io.Latent.Input(
                "latent",
                tooltip="Video latent, guides are added to the end of this latent",
            ),
            io.Image.Input(
                "multi_input", tooltip="Batched images from MultiImageLoader"
            ),
        ]
        ...
        return io.Schema(
            ...
            outputs=[
                ...
                io.Latent.Output(
                    display_name="latent", tooltip="Video latent with added guides"
                ),
            ],
        )
    ...
    def execute(
        ...
        for i in range(1, num_images + 1):
            ...
            img = multi_input[
                i - 1 : i
            ]  # Extract the single image frame from the batch
            if img is None:
                continue
            ...
            # Guard: skip frames whose latent index is entirely out of range
            if latent_idx >= latent_length:
                print(
                    f"[LTXSequencer] Warning: image {i} insert_frame={f_idx} → latent_idx={latent_idx} >= latent_length={latent_length}, skipping."
                )
                continue

            # Guard: clamp t to fit remaining latent frames — mirrors ltx_keyframer behavior
            if latent_idx + t.shape[2] > latent_length:
                available = latent_length - latent_idx
                print(
                    f"[LTXSequencer] Warning: image {i} insert_frame={f_idx} trimmed from {t.shape[2]} to {available} latent frames to fit within sequence (latent_length={latent_length})."
                )
                t = t[:, :, :available]
